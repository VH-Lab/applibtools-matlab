classdef log < handle
	% applibt.log: A logger for system and/or error messages by a Matlab application or package
	%
	% applibt.log is an API and implementation for writing messages to a log file. 
	...
	methods
		...
		function log_obj = log(varargin)
			% LOG - create a applibt.log for writing system, debugging, error information to log files
			%
			% LOG_OBJ = LOG(PROPERTY1, VALUE1, ... )
			%
			% Creates a new applibt.log object with the properties initiallized with name/value pair
				...
				switch upper(type),
					case 'SYSTEM',
						if priority >= log_obj.system_verbosity,
							[b,errormsg]=applibt.file.addline(log_obj.system_logfile, themsg);
						end;
					case 'ERROR',
						if priority >= log_obj.error_verbosity,
							[b,errormsg]=applibt.file.addline(log_obj.error_logfile, themsg);
						end;
					case 'DEBUG',
						if priority >= log_obj.debug_verbosity,
							[b,errormsg]=applibt.file.addline(log_obj.debug_logfile, themsg);
						end;
					otherwise,
