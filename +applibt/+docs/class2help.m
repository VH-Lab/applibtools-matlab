...
isclass = applibt.matlab.isclassfile(filename);

if ~isclass,
	error([filename ' does not appear to be a class file.']);
end;

packagename = applibt.matlab.mfile2package(filename);

classhelp_ = help(filename);
[classhelp,pos,linemarks] = applibt.string.line_n(classhelp_,1);
...
prop_struct = applibt.data.emptystruct('property','doc');
...
methods_struct = applibt.data.emptystruct('method','description','help');
