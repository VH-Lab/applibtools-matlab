...
%
% See also: applibt.docs.markdownoutput2objectstruct

out = applibt.data.emptystruct('title','path');
...
d = dir(input_path);
d = applibt.file.dirlist_trimdots(d); % only directories that are not hidden directories
...
for i=1:numel(w.m),
	...
	isclass = applibt.matlab.isclassfile([input_path filesep w.m{i}]);
	if isclass,
		classstr = 'CLASS ';
	else,
		classstr = '';
	...
	doctext = ['# ' classstr applibt.matlab.mfile2package([input_path filesep w.m{i}]) newline newline];
	...
	out_here.title = applibt.matlab.mfile2package([input_path filesep w.m{i}]);
	out_here.path = [ymlpath filesep w.m{i} '.md'];

	if isclass,
		[classhelp, prop_struct, methods_struct,superclassnames] = applibt.docs.class2help([input_path filesep w.m{i}]);
		...
		for j=1:numel(superclassnames),
			...
			linkclose = '';
			if ~isempty(index),
				linkhere = ['(' applibt.path.absolute2relative(objectstruct(index).path,out_here.path) ')'];
				linkopen = '[';
				linkclose = ']';
			end;
	...
	applibt.file.str2text([output_path filesep w.m{i} '.md'], doctext);
	out(end+1) = out_here;
end;
...
for i=1:numel(w.packages),
	...
	outst = applibt.docs.matlab2markdown(next_inputdir, next_outputdir, next_ymlpath, objectstruct, next_packageprefix);
...
for i=1:numel(d),
	...
	outst = applibt.docs.matlab2markdown(next_inputdir, next_outputdir, next_ymlpath, objectstruct);
