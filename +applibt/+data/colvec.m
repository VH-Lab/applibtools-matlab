...
%  (M = PROD(SIZE(X))).
% 
%  applibt.data.colvec is equivalent to Y = X(:);
%
%  This function is useful for addressing all of the elements of a matrix
%  as a vector when X is a subset of another matrix.
%
%  See also: applibt.data.rowvec
%
%  Example: 
%    A = rand(5,5)
%    Y = applibt.data.colvec(A(1:3,1:2)) % returns points in rows 1:3, columns 1:2
%
