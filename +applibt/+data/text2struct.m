...
%
%   S = applibt.data.text2struct(STR)
%
...
%
% mystruct = applibt.data.text2struct(str);
%
...
[dummy,dummy,eol_marks] = applibt.string.line_n(str,1);

bracedepth = applibt.string.bracelevel(str,BraceLeft,BraceRight);
...
while linesread<length(eol_marks),
	...
	[nextline,position] = applibt.string.line_n(str,1+linesread,'eol_marks',eol_marks),
	...
	while (~isempty(nextline) & linesread<length(eol_marks)), % while no end of structure mark
		...
		if stillgood,
			...
			if strcmp(value_str,'<'), % indicates a substructure
				...
				pos_match = applibt.string.bracematch(str,position+z-1,'braceleft','<','braceright','>','bracedepth',bracedepth);
				N = find(eol_marks>position+z-1,1,'first');
				%str(eol_marks(N)+1:pos_match-1),
				%double(str(eol_marks(N)+1:pos_match-1)),
				value = applibt.data.text2struct(str(eol_marks(N)+1:pos_match-1)),
