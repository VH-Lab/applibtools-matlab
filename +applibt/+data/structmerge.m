...
%
%  S_OUT = applibt.data.structmerge(S1, S2, ...)
%
...
if ErrorIfNewField,
	[c,f1i] = setdiff(f2,f1);
	if ~isempty(c),
		error(['Some fields of the second structure are not in the first: ' applibt.data.cell2str(c) '.']);
	end;
end;
...
if DoAlphabetical,
	...
	s_out = applibt.data.emptystruct(fn{:});
	for i=1:length(fn),
		if isempty(s_out),
