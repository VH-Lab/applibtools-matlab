...
%
% S = applibt.data.celltable2struct(C)
%
...
%    o = applibt.file.read_tab_delimited_file(filename);
%    s = applibt.data.celltable2struct(o);
%

fn = matlab.lang.makeValidName(c{1});

s = applibt.data.emptystruct(fn{:});

for i=2:numel(c),
	news = applibt.data.emptystruct(fn{:});
	for j=1:numel(fn),
		if j<=numel(c{i}),
