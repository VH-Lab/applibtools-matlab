...
% 
	...
	methods
		...
		function cache_obj = cache(varargin)
			...
			%
			% CACHE_OBJ = applibt.data.cache(...)
			%
			% Creates a new applibt.data.cache object. Additional arguments can be specified as
			% name value pairs:
			%
			...
			% See also: applibt.data.namevaluepair
				...
				if nargin==0,
					...
					cache_obj.table = applibt.data.emptystruct('key','type','timestamp','priority','bytes','data');
					return;
				end;
		...
		function cache_obj = set_replacement_rule(cache_obj, rule)
			...
			%
			% Sets the replacement rule for a applibt.data.cache object to be used when a new entry
		...
		function cache_obj = add(cache_obj, key, type, data, priority)
			...
			%
				...
				% now there's room
				newentry = applibt.data.emptystruct('key','type','timestamp','priority','bytes','data');
				newentry(1).key = key;
