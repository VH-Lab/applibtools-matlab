...
%
% applibt.file.filenamesearchreplace(DIRNAME, SEARCHSTRS, REPLACESTRS, ...)
%
...
%     applibt.file.filenamesearchreplace(mydirname,{'.ext1'},{'.ext2'}, 'deleteOriginals', 1)
...
d = applibt.file.dirstrip(dir(dirname)); % trim '.' and '..' and '.DS_store' and '.git'
...
for i=1:numel(d),
	...
	if d(i).isdir, % is a directory
		if recursive, % is a directory
			applibt.file.filenamesearchreplace([dirname filesep d(i).name],searchStrs, replaceStrs, varargin{:});
		end;
	end
end;
