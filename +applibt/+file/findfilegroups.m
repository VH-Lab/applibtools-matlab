...
%
%  FILELIST = applibt.file.findfilegroups(PARENTDIR, FILEPARAMETERS, ...)
%
...
%      fileparameters = {'.*\.ext\>'};
%      filelist = applibt.file.findfilegroups([ffg_prefix 'findfilegroupstest1'],fileparameters);
...
%      filelist = applibt.file.findfilegroups([ffg_prefix 'findfilegroupstest2'],fileparameters);
...
%      filelist = applibt.file.findfilegroups([ffg_prefix 'findfilegroupstest3'],fileparameters);
...
d = applibt.file.dirstrip(dir(parentdir));
...
if ~SearchParentFirst,
	for i=subdirs,
		filelist = cat(1,filelist,applibt.file.findfilegroups([parentdir filesep d(i).name], ...
...
if SearchParent,
	...
	filelist_potential = applibt.data.emptystruct('searchString','filelist');

	   % in order for a file group to pass, we have to find potential passing matches to the first criterion

	s2 = {d(regularfiles).name}';

	[tf, match_string, searchString] = applibt.string.strcmp_substitution(fileparameters{1}, s2, ...
	...
	for k=2:length(fileparameters),
		new_filelist_potential = applibt.data.emptystruct('searchString','filelist'); % we will add to this list
		for j=1:length(filelist_potential),
			[tf,match_string,newSearchString] = applibt.string.strcmp_substitution(fileparameters{k}, s2, ...
				'SubstituteStringSymbol', SameStringSearchSymbol, 'UseSubstiteString',UseSameStringSearchSymbol,...
