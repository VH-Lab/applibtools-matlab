...
%
%  [FID,KEY] = applibt.file.checkout_lock_file(FILENAME)
%
...
%  below. A KEY is returned, which is necessary to pass to to applibt.file.release_lock_file.
%
...
%  calls applibt.file.checkout_lock_file is able to create the file (that is, FID>0),
%  then it should call applibt.file.release_lock_file to remove the lock file.
%  
%  IMPORANT: FILE CLOSURE: If 2 output arguments are given (that is, KEY is
%  examined), then the lock file is closed before applibt.file.checkout_lock_file exits.
...
%  
%  [FID,KEY] = applibt.file.checkout_lock_file(FILENAME, CHECKLOOPS)
%
%  Alters the number of times the function will check (at 1 second
%  intervals) to see if FILENAME has disappeared.
%
%  [FID,KEY] = applibt.file.checkout_lock_file(FILENAME, CHECKLOOPS, THROWERROR)
%
%  If THROWERROR is 1, the function will return an error instead of
%  returning FID<0.
%
%  [FID,KEY] = applibt.file.checkout_lock_file(FILENAME, CHECKLOOPS, THROWERROR, EXPIRATION_SECONDS)
%
%  This mode allows one to specifically set the expiration time in seconds.
%  applibt.file.checkout_lock_file will examine the file for the expiration time and 
...
%     [lockfid,key] = applibt.file.checkout_lock_file(mylockfile);
%     if lockfid>0,
%        % do something
%        applibt.file.release_lock_file(mylockfile,key);
%     else,
%        error(['Never got control of ' mylockfile '; it was busy.']);
%     end;
%   
%
%  See also: applibt.file.release_lock_file, FOPEN, DELETE
