...
%
% A = applibt.file.loadStructArray(FNAME [, FIELDS])
%
...
%
% See also: applibt.file.saveStructArray, applibt.data.tabstr2struct
% 
...
while ~feof(fid),
	...
	if length(s)>0,
		...
		try,
			a(count) = applibt.data.tabstr2struct(s,fields);
		catch,
			fclose(fid);
