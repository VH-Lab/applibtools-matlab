...
%
%  NODE = applibt.file.xml.xmlstruct_findnode(XML_STRUCT, NODENAME)
%
%  Recursively searches XML_STRUCT, an XML node tree returned from
%  applibt.file.xml.parseXML, for the node with the name 'NODENAME'. It returns the node
...
% 
%  See also: applibt.file.xml.parseXML
%  
...
if any(gotit),
	...
	for i=1:length(xml_struct),
		node = applibt.file.xml.xmlstruct_findnode(xml_struct(i).Children,nodename);
		if ~isempty(node),
			return;
		end;
	end;
