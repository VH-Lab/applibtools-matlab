...
% described by minfo returned by applibt.matlab.mfileinfo.
%
...
%
% Returns the structure described in applibt.matlab.findfunctionusefile.
%
...
%                             |   begin with a '@' ?
%
% See also: applibt.matlab.mfileinfo, applibt.matlab.findfunctionusefile, applibt.data.namevaluepair
%
...
fuse = applibt.data.emptystruct('fullfilename','name','line','character','incomments','package_class_use','allcaps');

if numel(minfo)>1,
	for j=1:numel(minfo),
		fuse = cat(1,fuse(:),applibt.matlab.findfunctionusedir(dirname,minfo(j),varargin{:}));
	end;
	return;
end;
...
d = applibt.file.dirlist_trimdots(d,1);
...
for i=1:numel(d),
	if d(i).isdir, % a directory
		...
		fuse = cat(1,fuse(:),applibt.matlab.findfunctionusedir([dirname filesep d(i).name],minfo,varargin{:}));
	else, % a file
		[p,f,e] = fileparts(d(i).name);
		if strcmp(e,'.m'), % if we have a .m file, check it out
			fuse = cat(1,fuse(:), applibt.matlab.findfunctionusefile([dirname filesep d(i).name],minfo));
		end;
	end;
end;
