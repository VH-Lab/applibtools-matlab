...
%
%  INSIDE = applibt.matlab.graphics.selectpoints3d(PTS3D)
%
...
%     inside = applibt.matlab.graphics.selectpoints3d(pts'); % need to transpose to column vectors
%     hold on;
%     indexes = find(inside);
...
[x,y,z] = applibt.matlab.graphics.getline_3dplane;

[A] = applibt.matlab.graphics.viewpoint3dto2d( [x(:) y(:) z(:)]' );

if size(pts3d,1) == 2,
	pts3d = [pts3d; zeros(1,size(pts3d,2))];
end;

[B] = applibt.matlab.graphics.viewpoint3dto2d( pts3d );
