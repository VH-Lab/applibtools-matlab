...
%
% B = applibt.matlab.graphics.isinwhere(RECT, UNITS, WHERE)
%
...
if strcmp(where.units,'normalized'),
	if strcmp(units,'pixels'),
		rect=applibt.matlab.graphics.pixels2normalized(where.figure, rect);
	end;
elseif strcmp(where.units,'pixels'),
	if strcmp(units,'normalized'),
		rect=applibt.matlab.graphics.normalized2pixels(where.figure,rect);
	end;
end;
