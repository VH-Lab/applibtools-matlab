...
%   [X,Y,Z] = applibt.matlab.graphics.getline_3dplane(FIG) lets you select a polyline in the
...
%   polyline.
%
%   [X,Y,Z] = applibt.matlab.graphics.getline_3dplane(AX) lets you select a polyline in the axes
%   specified by the handle AX.
%
%   [X,Y,Z] = applibt.matlab.graphics.getline_3dplane is the same as [X,Y] = applibt.matlab.graphics.getline_3dplane(GCF).
%
%   [X,Y,Z] = applibt.matlab.graphics.getline_3dplane(...,'closed') animates and returns a closed
%   polygon.
%
%   Example
%   --------
%       imshow('moon.tif')
%       [x,y] = applibt.matlab.graphics.getline_3dplane 
%
...
%   Callback syntaxes:
%        applibt.matlab.graphics.getline_3dplane('KeyPress')
%        applibt.matlab.graphics.getline_3dplane('FirstButtonDown')
%        applibt.matlab.graphics.getline_3dplane('NextButtonDown')
%        applibt.matlab.graphics.getline_3dplane('ButtonMotion')
...
if ((nargin >= 1) && (ischar(varargin{end})))
    str = varargin{end};
    if (str(1) == 'c')
        % applibt.matlab.graphics.getline_3dplane(..., 'closed')
        GETLINE_ISCLOSED = 1;
...
if (length(varargin) < 1)
    ...
    if (~ishghandle(varargin{1}))
        ...
        error(message('applibt.matlab.graphics.getline_3dplane'));
    end
    ...
    otherwise
        CleanUp(xlimorigmode,ylimorigmode,zlimorigmode);
        error(message('applibt.matlab.graphics.getline_3dplane:expectedFigureOrAxesHandle'));
    end
end
...
set(GETLINE_FIG, ...
    'Pointer', 'crosshair', ...
    'WindowButtonDownFcn', 'applibt.matlab.graphics.getline_3dplane(''FirstButtonDown'');',...
    'KeyPressFcn', 'applibt.matlab.graphics.getline_3dplane(''KeyPress'');');
...
elseif (~ishghandle(GETLINE_H1) || ...
    ...
    % functions that call applibt.matlab.graphics.getline_3dplane.
    if (isempty(x))
        x = zeros(0,1);
    end
    if (isempty(y))
...
case 'trap'
    ...
    error(message('applibt.matlab.graphics.getline_3dplane:interruptedMouseSelection'));
...
if (~strcmp(get(GETLINE_FIG, 'SelectionType'), 'normal'))
    ...
    set(GETLINE_FIG, 'WindowButtonMotionFcn', 'applibt.matlab.graphics.getline_3dplane(''ButtonMotion'');', ...
            'WindowButtonDownFcn', 'applibt.matlab.graphics.getline_3dplane(''NextButtonDown'');');
end
