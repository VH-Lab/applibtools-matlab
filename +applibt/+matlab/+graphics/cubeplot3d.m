...
%
% applibt.matlab.graphics.cubeplot3d(NAME, 'data', DATA, ...)
%
% 
%
...
if ~command_extract_success,
	error(['Command must include plot name (see help applibt.matlab.graphics.cubeplot3d)']);
end;
...
elseif ~strcmp(lower(command),'get_vars') & ~strcmp(lower(command),'get_handles'), % let the routine below handle it
	ud = applibt.matlab.graphics.cubeplot3d(name,'command',[name 'Get_Vars'],'fig',fig);
end;

switch lower(command),
	case 'init',
		uidefs = applibt.ui.basicuitools_defs('callbackstr', ['applibt.ui.callbacknametag(''applibt.matlab.graphics.cubeplot3d'',''' name ''');']),
		...
		for i=1:numel(position),
			if strcmp(units,'pixels'),
				position{i} = applibt.matlab.graphics.normalized2pixels(fig,position{i});
			end
			axes('units',units,'position',position{i},'tag',[name position{i}]);
		end

		applibt.matlab.graphics.cubeplot3d(name,'command',[name 'Set_Vars'],'ud',ud);
