...
% 
% See also: applibt.matlab.mfileinfo
%
...
if numel(minfo)>1,
	for j=1:numel(minfo),
		fuse = cat(1,fuse(:),applibt.matlab.findfunctionusefile(filename,minfo(j)));
	end;
	return;
end;

full = applibt.file.fullfilename(filename);
t = applibt.file.text2cellstr(filename);
