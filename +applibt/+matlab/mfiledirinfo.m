...
% the same information as applibt.matlab.mfileinfo().
%
...
%
% 
% See also: applibt.matlab.mfileinfo(), applibt.data.namevaluepair()
%
% Example:
...
for i=1:numel(d),
	if d(i).isdir, % a directory
		...
		minfo_d = applibt.matlab.mfiledirinfo([dirname filesep d(i).name]);
		if isempty(minfo),
			minfo = minfo_d;
		else,
			minfo = [minfo(:); minfo_d(:)];
		...
		if strcmp(e,'.m'), % if we have a .m file, check it out
			minfo_f = applibt.matlab.mfileinfo([dirname filesep d(i).name]);
			if isempty(minfo),
				minfo = minfo_f;
			else,
				minfo(end+1,:) = minfo_f;
