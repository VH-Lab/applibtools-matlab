...
% returned from applibt.matlab.findfunctionusefil or applibt.matlab.findfunctionusedir,
...
%
% See also: applibt.matlab.findfunctionusefile, applibt.matlab.mfileinfo
%
%
...
for f=1:numel(fuse),
	...
	t = applibt.file.text2cellstr(fuse(f).fullfilename);
	...
	switch lower(reply),
		case 'y',
			...
			t{fuse(f).line},
			if ~Disable,
				applibt.file.cellstr2text(fuse(f).fullfilename, t);
